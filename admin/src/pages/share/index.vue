...
const mockShops = ref<Shop[]>([
  {
    id: 1, name: '老张牛肉面馆', category: '面馆', rating: 4.8, distance: '1.2km', price: '人均 ¥25',
    tags: ['牛肉面', '老字号', '分量足'],
    description: '开了二十年的老店，牛肉软烂入味，汤头浓郁，面条劲道。每次来都要排队，但绝对值得！',
    author: '小王', date: '07-01', likes: 128, comments: 32, cover: '',
    commentsList: [
  ...
  {
    id: 2, name: '巷子深 · 精品咖啡馆', category: '咖啡/轻食', rating: 4.6, distance: '0.8km', price: '人均 ¥38',
    tags: ['手冲', '精品豆', '安静'],
    description: '藏在居民区里的宝藏咖啡馆，老板是SCA认证咖啡师。手冲耶加雪菲花香四溢，环境安静适合自习。',
    author: '小李', date: '06-28', likes: 89, comments: 15, cover: '', commentsList: [],
  },
  {
    id: 3, name: '蜀味轩川菜馆', category: '川菜/快餐', rating: 4.5, distance: '2.1km', price: '人均 ¥55',
    tags: ['水煮鱼', '麻辣', '聚餐'],
    description: '正宗川味，水煮鱼片嫩滑，麻婆豆腐下饭神器。适合教研室聚餐，量大实惠。',
    author: '小张', date: '06-25', likes: 203, comments: 48, cover: '',
    commentsList: [
  ...
  {
    id: 4, name: '阿美奶茶铺', category: '甜品/奶茶', rating: 4.7, distance: '0.5km', price: '人均 ¥16',
    tags: ['珍珠奶茶', '手作', '性价比'],
    description: '现煮黑糖珍珠Q弹有嚼劲，奶盖咸甜适中。下午茶首选，外卖也很快。',
    author: '小陈', date: '07-02', likes: 156, comments: 23, cover: '', commentsList: [],
  },
  {
    id: 5, name: '韩式炸鸡啤酒屋', category: '韩料', rating: 4.4, distance: '1.8km', price: '人均 ¥48',
    tags: ['炸鸡', '啤酒', '夜宵'],
    description: '韩式甜辣炸鸡外酥里嫩，配上冰啤酒简直完美。加班后的深夜食堂。',
    author: '小赵', date: '06-30', likes: 176, comments: 41, cover: '', commentsList: [],
  },
  {
    id: 6, name: '素食记', category: '素食', rating: 4.3, distance: '1.5km', price: '人均 ¥32',
    tags: ['素食', '健康', '清新'],
    description: '精致的素食小馆，口味清淡却不寡淡，推荐招牌素狮子头和菌菇汤。',
    author: '小周', date: '06-27', likes: 67, comments: 12, cover: '', commentsList: [],
  },
]);
...
const filteredShops = computed(() => {
  ...
  return mockShops.value.filter(
    (s) => s.name.toLowerCase().includes(q) || s.tags.some((t) => t.toLowerCase().includes(q)) || s.category.toLowerCase().includes(q),
  );
});

function likeShop(shop: Shop) { shop.likes++; }
...
function toggleDetailLike() {
  ...
  if (activeShop.value.isLiked) { activeShop.value.isLiked = false; activeShop.value.likes--; }
  else { activeShop.value.isLiked = true; activeShop.value.likes++; }
}
