  ...
  >
    <div class="detail-container" v-if="shop">
      ...
      <div class="detail-left">
        <div v-if="shop.cover" class="detail-image" :style="{ backgroundImage: `url(${shop.cover})` }" />
      ...
      <div class="detail-right">
        ...
        <div class="detail-footer">
          <div class="detail-actions">
            <button @click="$emit('toggleLike')" class="action-btn" :class="{ liked: shop.isLiked }">
          ...
          <div class="comment-input-row">
            ...
            />
            <el-button type="primary" link :disabled="!commentText?.trim()" @click="$emit('submitComment')">
              发送
            </el-button>
          </div>
        </div>
...
import { Close, Picture, StarFilled, Location, Pointer, ChatLineRound } from '@element-plus/icons-vue';
...
<style scoped>
.detail-container { display: flex; height: 650px; background: #fff; position: relative; }
...
.detail-left { width: 60%; background: #111827; display: flex; align-items: center; justify-content: center; }
.detail-image { width: 100%; height: 100%; background-size: cover; background-position: center; }
.detail-image-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; color: #6b7280; gap: 1rem; }

.detail-right { width: 40%; display: flex; flex-direction: column; border-left: 1px solid #f3f4f6; }
.detail-author { display: flex; align-items: center; gap: 0.75rem; padding: 1.25rem; border-bottom: 1px solid #f3f4f6; flex-shrink: 0; }
...
.rating { display: flex; align-items: center; gap: 0.25rem; background: #fff7ed; padding: 0.125rem 0.5rem; border-radius: 0.25rem; border: 1px solid #fed7aa; }
.rating span { font-weight: 700; color: #ea580c; }
.rating-lg { font-size: 0.875rem; }

.detail-desc { color: #374151; font-size: 0.938rem; line-height: 1.8; white-space: pre-wrap; margin: 0 0 1.5rem; }
...
.detail-footer { padding: 1rem 1.25rem; border-top: 1px solid #f3f4f6; background: #fff; flex-shrink: 0; }
.detail-actions { display: flex; gap: 1.5rem; margin-bottom: 1rem; padding: 0 0.5rem; }
.action-btn {
  display: flex; align-items: center; gap: 0.5rem;
  border: none; background: transparent; cursor: pointer;
  color: #4b5563; font-size: 1.125rem; padding: 0; transition: color 0.2s;
}
...
.comment-input-row {
  ...
  padding: 0.25rem 1rem; transition: border-color 0.2s, background 0.2s;
}
